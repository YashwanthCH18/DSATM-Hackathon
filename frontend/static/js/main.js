...
function updateUIForAuthStatus() {
    ...
    if (signupBtn && loginBtn && logoutBtn) {
        if (isAuthenticated) {
            ...
            // User is not logged in - show signup/login, hide logout
            signupBtn.style.display = 'none'; // Hide signup, only show login
...
function logoutUser() {
    ...
    axios.post('/api/logout')
        ...
        .catch(error => {
            ...
            // Even if the API call fails, clear local storage and update UI
            localStorage.removeItem('user');
            isAuthenticated = false;
            updateUIForAuthStatus();
